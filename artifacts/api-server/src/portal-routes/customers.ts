...
export async function registerCustomerRoutes(app: Express, _httpServer: Server) {
  ...
  app.get("/api/customers", isAuthenticated, asyncHandler(async (req, res) => {
  ...
  app.get("/api/customers/:id", isAuthenticated, asyncHandler(async (req, res) => {
  ...
  app.post("/api/customers", isAuthenticated, asyncHandler(async (req, res) => {
    ...
    const parsed = { data: customerSchema.parse(req.body) } as const;
    ...
    if (emailExists) {
      throw HttpError.badRequest("A customer with this email already exists");
    }
  ...
  // Update customer
  app.patch("/api/customers/:id", isAuthenticated, asyncHandler(async (req, res) => {
    const user = (req as any).managedUser as ManagedUser;
  ...
  // Delete customer
  app.delete("/api/customers/:id", isAuthenticated, isAdmin, asyncHandler(async (req, res) => {
    const user = (req as any).managedUser as ManagedUser;
    const existing = await storage.getCustomer(req.params.id);
  ...
  app.post("/api/customers/import/preview", isAuthenticated, isSuperAdmin, upload.single("file"), asyncHandler(async (req, res) => {
  ...
  app.post("/api/customers/import", isAuthenticated, isSuperAdmin, asyncHandler(async (req, res) => {
    const user = (req as any).managedUser as ManagedUser;
    const { fileData, mapping } = req.body;

    if (!fileData || !mapping) {
      throw HttpError.badRequest("File data and column mapping are required");
    }

    if (!mapping.firstName && !mapping.lastName && !mapping.email && !mapping.contact) {
      throw HttpError.badRequest("At least one field mapping is required");
    }
  ...
  app.post("/api/customers/import-log", isAuthenticated, asyncHandler(async (req, res) => {
    const user = (req as any).managedUser as ManagedUser;
  ...
  app.get("/api/customers/import-logs", isAuthenticated, asyncHandler(async (req, res) => {
  ...
  app.post("/api/customers/duplicates/scan", isAuthenticated, asyncHandler(async (req, res) => {
    const parsed = { data: duplicateScanSchema.parse(req.body) } as const;
  ...
  app.post("/api/customers/merge", isAuthenticated, isAdmin, asyncHandler(async (req, res) => {
    const user = (req as any).managedUser as ManagedUser;
    const parsed = { data: customerMergeSchema.parse(req.body) } as const;
    const { primaryId, secondaryIds } = parsed.data;

    const primary = await storage.getCustomer(primaryId);
    if (!primary) {
      throw HttpError.notFound("Primary record not found");
    }
  ...
  app.delete("/api/customers/duplicates/bulk", isAuthenticated, isAdmin, asyncHandler(async (req, res) => {
    const user = (req as any).managedUser as ManagedUser;
    const { ids } = req.body;

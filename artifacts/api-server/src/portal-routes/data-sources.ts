...
export async function registerDataSourceRoutes(app: Express, _httpServer: Server) {
  ...
  app.get("/api/data-sources", isAuthenticated, asyncHandler(async (req, res) => {
  ...
  app.get("/api/data-sources/:slug", isAuthenticated, asyncHandler(async (req, res) => {
    const ds = await storage.getDataSourceBySlug(req.params.slug);
    if (!ds) throw HttpError.notFound("Data source not found");
    res.json(ds);
  }));

  app.get("/api/data-sources/:slug/records", isAuthenticated, asyncHandler(async (req, res) => {
    const ds = await storage.getDataSourceBySlug(req.params.slug);
    if (!ds) throw HttpError.notFound("Data source not found");
  ...
  app.post("/api/data-sources/:slug/import/preview", isAuthenticated, isSuperAdmin, dsUpload.single("file"), asyncHandler(async (req, res) => {
    const ds = await storage.getDataSourceBySlug(req.params.slug);
    if (!ds) throw HttpError.notFound("Data source not found");
    ...
    if (![".xlsx", ".csv"].some(e => ext.endsWith(e))) {
      throw HttpError.badRequest("Invalid file type. Please upload .xlsx or .csv");
    }
  ...
  app.post("/api/data-sources/:slug/import", isAuthenticated, isSuperAdmin, asyncHandler(async (req, res) => {
    const ds = await storage.getDataSourceBySlug(req.params.slug);
    if (!ds) throw HttpError.notFound("Data source not found");

    const user = (req as any).managedUser as ManagedUser;
    const parsed = { data: dsImportSchema.parse(req.body) } as const;
  ...
  app.get("/api/data-sources/:slug/import-logs", isAuthenticated, asyncHandler(async (req, res) => {
    const ds = await storage.getDataSourceBySlug(req.params.slug);
    if (!ds) throw HttpError.notFound("Data source not found");
  ...
  app.post("/api/data-sources/:slug/duplicates/scan", isAuthenticated, asyncHandler(async (req, res) => {
    const ds = await storage.getDataSourceBySlug(req.params.slug);
    if (!ds) throw HttpError.notFound("Data source not found");

    const parsed = { data: dsDuplicateScanSchema.parse(req.body) } as const;
  ...
  app.post("/api/data-sources/:slug/merge", isAuthenticated, isSuperAdmin, asyncHandler(async (req, res) => {
    const parsed = { data: dsMergeSchema.parse(req.body) } as const;
    const { primaryId, secondaryIds } = parsed.data;

    const primary = await storage.getDsRecord(primaryId);
    if (!primary) throw HttpError.notFound("Primary record not found");
  ...
  app.delete("/api/data-sources/:slug/records", isAuthenticated, isSuperAdmin, asyncHandler(async (req, res) => {
    const { ids } = req.body;
    if (!ids || !Array.isArray(ids) || ids.length === 0) {
      throw HttpError.badRequest("At least one ID is required");
    }
  ...
  app.patch("/api/data-sources/:slug/records/:id", isAuthenticated, isSuperAdmin, asyncHandler(async (req, res) => {
    const parsed = { data: dsRecordUpdateSchema.parse(req.body) } as const;
  ...
  app.post("/api/data-sources/:slug/records/delete-batch", isAuthenticated, isSuperAdmin, asyncHandler(async (req, res) => {
    const { ids } = req.body;
    if (!ids || !Array.isArray(ids) || ids.length === 0) {
      throw HttpError.badRequest("At least one ID is required");
    }
  ...
  app.delete("/api/data-sources/:slug/records/all", isAuthenticated, isSuperAdmin, asyncHandler(async (req, res) => {
    const ds = await storage.getDataSourceBySlug(req.params.slug);
    if (!ds) throw HttpError.notFound("Data source not found");
  ...
  app.post("/api/data-sources/:slug/records/clear-all", isAuthenticated, isSuperAdmin, asyncHandler(async (req, res) => {
    const ds = await storage.getDataSourceBySlug(req.params.slug);
    if (!ds) throw HttpError.notFound("Data source not found");
    const result = await db.delete(dsRecords).where(eq(dsRecords.dataSourceId, ds.id));
    await db.update(dataSources).set({ recordCount: 0 }).where(eq(dataSources.id, ds.id));
  ...
  app.patch("/api/data-sources/:slug/settings", isAuthenticated, isAdmin, asyncHandler(async (req, res) => {
    const ds = await storage.getDataSourceBySlug(req.params.slug);
    if (!ds) throw HttpError.notFound("Data source not found");

    const parsed = { data: dsSettingsSchema.parse(req.body) } as const;
